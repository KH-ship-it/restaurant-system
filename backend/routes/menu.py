...
# ======================================================
# 📌 GET PUBLIC MENU (FIX 405 – CHO PHÉP / & KHÔNG /)
# ======================================================
@router.get("/public")
@router.get("/public/")
...
def create_menu_item(
    ...
    cursor = conn.cursor(cursor_factory=RealDictCursor)
    try:
        status_value = item.status.upper() if item.status else "AVAILABLE"
        ...
        conn.rollback()
        cursor.close()
        print("🔥 CREATE MENU ERROR:", e)  # <<< RẤT QUAN TRỌNG
